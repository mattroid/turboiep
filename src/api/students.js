...
studentService.before({
  create(hook) {
    if (hook.data.questions || hook.data.questions.length === 0) {
      hook.data.headerItems = getDefaultHeaderItems(); // eslint-disable-line no-param-reassign, no-use-before-define, max-len
      hook.data.questions = getDefaultMathQuestions(); // eslint-disable-line no-param-reassign, no-use-before-define, max-len
    }
  },
});
studentService.after({
  find(hook, next) {
    ...
    answerService.find({ query: { studentId } }).then((answers) => {
      // map answers to questions
      hook.result[0].questions.forEach((question) => {
        answers.forEach((answer) => {
          if (answer.questionId === question.questionId) {
            question.questionItems.forEach((qi) => {
              const qitem = qi
